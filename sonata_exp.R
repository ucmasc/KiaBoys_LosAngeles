# Hyundai Sonata theft model (exponential specification)
# Alternative to sonata.R for Table 3 model comparison.
# Uses exp(i*d) instead of i^d for market growth.
...
model <- suppressMessages(stan_model("sonata_exp.stan"))
...
for (i in 1:48) {
  for (j in 1:50) {
    if ((i - j) < 26) {
      ...
      if (i < 36) {
        lam[i, j] <- exp(i * d) * carseed / scrap
      } else if (j > 20 && i < 45) {
        lam[i, j] <- exp(i * d) * bump * (1 / (1 + exp(-r0 * (j - 20 - x0)))) * carseed / scrap
      } else {
        lam[i, j] <- exp(i * d) * carseed / scrap
      }
    }
...
myplot <- function(j) {
  ggplot() +
    ...
    ggtitle(paste0("Sonata (exp) Year: ", j + 1999)) + theme_bw()
}
plist <- lapply(1:24, myplot)
p <- grid.arrange(grobs = plist, ncol = 5)
#ggsave("model_output/sonata_exp_plot.pdf", p, width = 15, height = 10)
