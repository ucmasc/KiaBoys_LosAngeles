# Kia Optima theft model (Appendix D)
# Fits Bayesian Poisson model via Stan and generates:
#   - model_optima.csv: fitted values for 2000-2023
#   - optima_forecast.csv: 50-year forecast matrix
#   - optima_plot.pdf: observed vs fitted by theft year
#   - optima_forecast.pdf: forecast heatmap (similar to Figure 9)
...
thefts <- read.csv("data/optima_thefts_by_year.csv")
CarMat <- as.matrix(thefts[1:48, 3:26])

# Fit Stan model
model <- suppressMessages(stan_model("optima.stan"))
...
# i>=36 & i<46 corresponds to 2011-2020 model-years (vulnerable)
...
# Fitted values for observation period
lam2 <- lam[, 1:24]
colnames(lam2) <- seq(2000, 2023)
rownames(lam2) <- thefts[1:48, 2]
df <- melt(lam2)
colnames(df) <- c("y", "x", "thefts")
write.csv(lam2, "model_output/model_optima_final.csv")

# MASE for Table 4
cat("***Optima model (power-law)***\n")
...
myplot <- function(j) {
  ggplot() +
    ...
    ggtitle(paste0("Optima Theft Year: ", j + 1999)) + theme_bw()
}
plist <- lapply(1:24, myplot)
p <- grid.arrange(grobs = plist, ncol = 5)
#ggsave("model_output/optima_plot.pdf", p, width = 15, height = 10)

# Forecast heatmap (Figure 9)
...
#ggsave("model_output/optima_forecast.pdf", p2, width = 8, height = 5)

write.csv(lam, "model_output/optima_forecast_final.csv")
