# Toyota Camry theft model
# Fits Bayesian Poisson model via Stan and generates:
#   - model_camry.csv: fitted values for 2000-2023
#   - camry_plot.pdf: observed vs fitted by theft year
...
thefts <- read.csv("data/camry_thefts_by_year.csv")
CarMat <- as.matrix(thefts[1:48, 3:26])

# Fit Stan model
model <- suppressMessages(stan_model("camry.stan"))
...
# i<17 corresponds to pre-1992 model-years (no immobilizer)
# i>=17 corresponds to 1992+ model-years (with immobilizer)
lam <- matrix(0, 48, 24)
for (i in 1:48) {
  for (j in 1:24) {
    if ((i - j) < 26) {
...
write.csv(lam2, "model_output/model_camry_final.csv")
...
myplot <- function(j) {
  ggplot() +
    ...
    ggtitle(paste0("Camry Theft Year: ", j + 1999)) + theme_bw()
}
plist <- lapply(1:24, myplot)
p <- grid.arrange(grobs = plist, ncol = 5)
#ggsave("data/camry_plot.pdf", p, width = 15, height = 10)
